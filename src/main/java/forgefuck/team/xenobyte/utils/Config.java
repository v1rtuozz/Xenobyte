...
public class Config {
    ...
    private static Timer configReloadTimer;
    private static long lastModifiedTime; // [!] Храним время последнего изменения
    ...
    public static void save() {
        ...
        lastModifiedTime = configFile.lastModified(); // [!] Обновляем время после сохранения
    }
    
    public static void load() {
        try {
            long currentModifiedTime = configFile.lastModified(); // [!] Получаем текущее время
            if (currentModifiedTime > lastModifiedTime) { // [!] Проверяем, изменился ли файл
                data = gson.fromJson(fileProvider.readFile(), ConfigData.class);
                lastModifiedTime = currentModifiedTime; // [!] Обновляем время после загрузки
                Xeno.logger.info("Config reloaded (file changed)");
            }
    ...
    static {
        ...
        lastModifiedTime = configFile.lastModified(); // [!] Инициализируем время
        if (firstStart) {
            save();
        } else {
            load();
        ...
        // [!] Таймер с проверкой изменений
        configReloadTimer = new Timer();
        configReloadTimer.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
                load(); // Будет грузиться только если файл изменён
            }
        }, 1000, 1000); // Проверка каждую секунду
    }
